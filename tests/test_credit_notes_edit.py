"""Tests for the credit note edit form — Lane D cycle 15.

Eight tests:
1. test_edit_requires_auth              — GET /credit-notes/{id}/edit without session -> 303 /login
2. test_edit_form_renders_for_draft     — mock DRAFT credit note -> form with version hidden input + lines
3. test_edit_blocked_for_posted         — mock POSTED credit note -> blocked page (422), no form
...
Divergences from invoice/bill tests:
- No due_date or payment_terms in form data (CreditNoteUpdate has neither).
- credit_note.reason field present and editable.
- original_invoice_id field present and editable.
- Conflict banner says "Someone else updated this credit note."
"""
...
_CN_ID = "ffffffff-ffff-ffff-ffff-ffffffffffff"
...
_MOCK_CN_POSTED = {**_MOCK_CN_DRAFT, "status": "POSTED", "version": 4}

# A newer server version returned after a 409 conflict.
_MOCK_CN_V4 = {**_MOCK_CN_DRAFT, "version": 4, "notes": "Updated by someone else"}
...
async def test_edit_requires_auth() -> None:
    """GET /credit-notes/{id}/edit without a session redirects to /login (303)."""
    async with AsyncClient(
        transport=ASGITransport(app=app),
        base_url="http://test",
        follow_redirects=False,
    ) as client:
        resp = await client.get(f"/credit-notes/{_CN_ID}/edit")
...
# ---------------------------------------------------------------------------
# 2. Edit form renders for DRAFT credit note
# ---------------------------------------------------------------------------
...
async def test_edit_form_renders_for_draft(respx_mock: respx.MockRouter) -> None:
    """GET /credit-notes/{id}/edit for a DRAFT credit note renders the edit form."""
    ...
    assert "Returned goods" in resp.text
...
# ---------------------------------------------------------------------------
# 3. Edit blocked for POSTED credit note
# ---------------------------------------------------------------------------
...
async def test_edit_blocked_for_posted(respx_mock: respx.MockRouter) -> None:
    """GET /credit-notes/{id}/edit for a POSTED credit note shows blocked page, not the form."""
    respx_mock.get(f"{_API_BASE}/api/v1/credit_notes/{_CN_ID}").mock(
...
async def test_edit_success_redirects(respx_mock: respx.MockRouter) -> None:
    """POST /credit-notes/{id}/edit with valid body; API 200 -> 303 to detail page."""
    respx_mock.patch(f"{_API_BASE}/api/v1/credit_notes/{_CN_ID}").mock(
        return_value=Response(200, json=_MOCK_CN_DRAFT)
    )
    ...
    async with AsyncClient(
        ...
        resp = await client.post(
            ...
            data={
                ...
                "lines[0][description]": "Returned goods",
                "lines[0][quantity]": "1",
...
async def test_edit_sends_if_match_header(respx_mock: respx.MockRouter) -> None:
    """POST /credit-notes/{id}/edit passes the version as the If-Match header."""
    ...
    def _capture(request: respx.Request) -> Response:
        ...
        return Response(200, json=_MOCK_CN_DRAFT)

    respx_mock.patch(f"{_API_BASE}/api/v1/credit_notes/{_CN_ID}").mock(side_effect=_capture)
...
async def test_edit_conflict_shows_banner(respx_mock: respx.MockRouter) -> None:
    ...
    # The route re-fetches the credit note after 409 to get the latest version.
    respx_mock.get(f"{_API_BASE}/api/v1/credit_notes/{_CN_ID}").mock(
        return_value=Response(200, json=_MOCK_CN_V4)
    )
    _mock_dropdowns(respx_mock)
    ...
    assert "Someone else updated this credit note" in resp.text
...
async def test_edit_validation_error(respx_mock: respx.MockRouter) -> None:
    """POST /credit-notes/{id}/edit where API returns 422 re-renders form with errors."""
    _422_body = {
        "detail": [
            {
                "type": "missing",
...
async def test_edit_parses_lines_replacement(respx_mock: respx.MockRouter) -> None:
    ...
    async with AsyncClient(
        ...
        await client.post(
            ...
            data={
                ...
                "lines[1][description]": "Freight refund",
                "lines[1][quantity]": "1",
                "lines[1][unit_price]": "25.00",
                # Line 2
                "lines[2][account_id]": _ACCOUNT_ID3,
                "lines[2][description]": "Restocking fee",
                "lines[2][quantity]": "3",
    ...
    assert lines[0]["description"] == "Returned goods"
    assert lines[1]["description"] == "Freight refund"
    assert lines[2]["description"] == "Restocking fee"
    assert lines[0]["unit_price"] == "50.00"
    assert lines[1]["unit_price"] == "25.00"
    assert lines[2]["unit_price"] == "10.00"
