"""Tests for the bill edit form — Lane D cycle 13.
...
_BILL_ID = "ffffffff-ffff-ffff-ffff-ffffffffffff"
...
_MOCK_CONTACT = {"id": _CONTACT_ID, "name": "Acme Supplies Pty Ltd", "contact_type": "SUPPLIER"}
_MOCK_CONTACTS = {"items": [_MOCK_CONTACT], "total": 1, "limit": 500, "offset": 0}

_MOCK_ACCOUNT = {"id": _ACCOUNT_ID, "name": "Office Supplies", "code": "6200", "account_type": "EXPENSE"}
_MOCK_ACCOUNTS = {"items": [_MOCK_ACCOUNT], "total": 1, "limit": 500, "offset": 0}

_MOCK_TAX_CODE = {"id": _TAX_CODE_ID, "name": "GST", "rate": "0.10"}
_MOCK_TAX_CODES = {"items": [_MOCK_TAX_CODE], "total": 1, "page_size": 500}
...
async def test_edit_requires_auth() -> None:
    """GET /bills/{id}/edit without a session redirects to /login (303)."""
    async with AsyncClient(
        transport=ASGITransport(app=app),
        base_url="http://test",
        follow_redirects=False,
    ) as client:
        resp = await client.get(f"/bills/{_BILL_ID}/edit")
...
# ---------------------------------------------------------------------------
# 2. Edit form renders for DRAFT bill
# ---------------------------------------------------------------------------
...
async def test_edit_form_renders_for_draft(respx_mock: respx.MockRouter) -> None:
    """GET /bills/{id}/edit for a DRAFT bill renders the edit form."""
    ...
    assert "Stationery" in resp.text
...
async def test_edit_success_redirects(respx_mock: respx.MockRouter) -> None:
    """POST /bills/{id}/edit with valid body; API 200 -> 303 to detail page."""
    respx_mock.patch(f"{_API_BASE}/api/v1/bills/{_BILL_ID}").mock(
        return_value=Response(200, json=_MOCK_BILL_DRAFT)
    )
    ...
    async with AsyncClient(
        ...
        resp = await client.post(
            ...
            data={
                ...
                "lines[0][description]": "Stationery",
                "lines[0][quantity]": "1",
...
async def test_edit_sends_if_match_header(respx_mock: respx.MockRouter) -> None:
    """POST /bills/{id}/edit passes the version as the If-Match header."""
    ...
    def _capture(request: respx.Request) -> Response:
        ...
        return Response(200, json=_MOCK_BILL_DRAFT)

    respx_mock.patch(f"{_API_BASE}/api/v1/bills/{_BILL_ID}").mock(side_effect=_capture)
...
async def test_edit_conflict_shows_banner(respx_mock: respx.MockRouter) -> None:
    ...
    # The route re-fetches the bill after 409 to get the latest version.
    respx_mock.get(f"{_API_BASE}/api/v1/bills/{_BILL_ID}").mock(
        return_value=Response(200, json=_MOCK_BILL_V4)
    )
    _mock_dropdowns(respx_mock)
    ...
    assert "Someone else updated this bill" in resp.text
...
async def test_edit_validation_error(respx_mock: respx.MockRouter) -> None:
    """POST /bills/{id}/edit where API returns 422 re-renders form with errors."""
    _422_body = {
        "detail": [
            {
                "type": "missing",
...
async def test_edit_parses_lines_replacement(respx_mock: respx.MockRouter) -> None:
    ...
    async with AsyncClient(
        ...
        await client.post(
            ...
            data={
                ...
                "lines[1][description]": "Freight",
                "lines[1][quantity]": "1",
                "lines[1][unit_price]": "25.00",
                # Line 2
                "lines[2][account_id]": _ACCOUNT_ID3,
                "lines[2][description]": "Packaging",
                "lines[2][quantity]": "3",
    ...
    assert lines[0]["description"] == "Stationery"
    assert lines[1]["description"] == "Freight"
    assert lines[2]["description"] == "Packaging"
