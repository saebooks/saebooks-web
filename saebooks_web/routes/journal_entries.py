...
# ---------------------------------------------------------------------------
# Bulk action — POST /journal-entries/bulk
# ---------------------------------------------------------------------------

_BULK_ACTIONS_JOURNAL_ENTRIES = {
    "post": ("POST", "/api/v1/journal_entries/{id}/post"),
    "reverse": ("POST", "/api/v1/journal_entries/{id}/reverse"),
    "archive": ("DELETE", "/api/v1/journal_entries/{id}"),
}


@router.post("/journal-entries/bulk", response_class=HTMLResponse, response_model=None)
async def journal_entries_bulk_action(request: Request) -> RedirectResponse:
    """Run an action against many journal entrys at once.

    Form fields:
      action  — one of: post, reverse, archive
      ids[]   — one entry per UUID

    Aggregates per-row outcomes into a flash message and redirects back
    to /journal-entries. Best-effort: a failed row does not halt the batch.
    """
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
    ...
    if action not in _BULK_ACTIONS_JOURNAL_ENTRIES:
        request.session["flash"] = f"Unknown bulk action: {action!r}"
        return RedirectResponse(url="/journal-entries", status_code=303)
    ...
    method, path_tpl = _BULK_ACTIONS_JOURNAL_ENTRIES[action]
    ok = 0
    failed: list[str] = []
    async with api_client(request) as client:
        for row_id in ids:
    ...
    if failed:
        ...
        request.session["flash"] = f"{label}: {ok} journal entry{'s' if ok != 1 else ''} processed."
    return RedirectResponse(url="/journal-entries", status_code=303)
...
@router.post("/journal-entries/{entry_id}/hard-delete", response_class=HTMLResponse, response_model=None)
async def journal_entry_hard_delete(request: Request, entry_id: str) -> RedirectResponse:
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
