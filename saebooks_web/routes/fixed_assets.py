...
# ---------------------------------------------------------------------------
# Bulk action — POST /fixed-assets/bulk
# ---------------------------------------------------------------------------

_BULK_ACTIONS_FIXED_ASSETS = {
    "archive": ("DELETE", "/api/v1/fixed_assets/{id}"),
}


@router.post("/fixed-assets/bulk", response_class=HTMLResponse, response_model=None)
async def fixed_assets_bulk_action(request: Request) -> RedirectResponse:
    """Run an action against many fixed assets at once.
    ...
    to /fixed-assets. Best-effort: a failed row does not halt the batch.
    """
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
    ...
    if action not in _BULK_ACTIONS_FIXED_ASSETS:
        request.session["flash"] = f"Unknown bulk action: {action!r}"
        return RedirectResponse(url="/fixed-assets", status_code=303)
    ...
    method, path_tpl = _BULK_ACTIONS_FIXED_ASSETS[action]
    ok = 0
    failed: list[str] = []
    async with api_client(request) as client:
        for row_id in ids:
    ...
    if failed:
        ...
        request.session["flash"] = f"{label}: {ok} fixed asset{'s' if ok != 1 else ''} processed."
    return RedirectResponse(url="/fixed-assets", status_code=303)
...
@router.post("/fixed-assets/{asset_id}/hard-delete", response_class=HTMLResponse, response_model=None)
async def fixed_asset_hard_delete(request: Request, asset_id: str) -> RedirectResponse:
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
