...
# ---------------------------------------------------------------------------
# Bulk action — POST /budgets/bulk
# ---------------------------------------------------------------------------

_BULK_ACTIONS_BUDGETS = {
    "archive": ("DELETE", "/api/v1/budgets/{id}"),
}


@router.post("/budgets/bulk", response_class=HTMLResponse, response_model=None)
async def budgets_bulk_action(request: Request) -> RedirectResponse:
    """Run an action against many budgets at once.
    ...
    to /budgets. Best-effort: a failed row does not halt the batch.
    """
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
    ...
    if action not in _BULK_ACTIONS_BUDGETS:
    ...
    method, path_tpl = _BULK_ACTIONS_BUDGETS[action]
    ok = 0
    failed: list[str] = []
    async with api_client(request) as client:
        for row_id in ids:
    ...
    if failed:
        ...
        request.session["flash"] = f"{label}: {ok} budget{'s' if ok != 1 else ''} processed."
    return RedirectResponse(url="/budgets", status_code=303)
...
@router.post("/budgets/{budget_id}/hard-delete", response_class=HTMLResponse, response_model=None)
async def budget_hard_delete(request: Request, budget_id: str) -> RedirectResponse:
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
