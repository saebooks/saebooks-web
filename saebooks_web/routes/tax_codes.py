...
# ---------------------------------------------------------------------------
# Bulk action — POST /tax-codes/bulk
# ---------------------------------------------------------------------------

_BULK_ACTIONS_TAX_CODES = {
    "archive": ("DELETE", "/api/v1/tax_codes/{id}"),
}


@router.post("/tax-codes/bulk", response_class=HTMLResponse, response_model=None)
async def tax_codes_bulk_action(request: Request) -> RedirectResponse:
    """Run an action against many tax codes at once.
    ...
    to /tax-codes. Best-effort: a failed row does not halt the batch.
    """
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
    ...
    if action not in _BULK_ACTIONS_TAX_CODES:
    ...
    method, path_tpl = _BULK_ACTIONS_TAX_CODES[action]
    ok = 0
    failed: list[str] = []
    async with api_client(request) as client:
        for row_id in ids:
    ...
    if failed:
        ...
        request.session["flash"] = f"{label}: {ok} tax code{'s' if ok != 1 else ''} processed."
    return RedirectResponse(url="/tax-codes", status_code=303)
...
@router.post("/tax-codes/{tax_code_id}/hard-delete", response_class=HTMLResponse, response_model=None)
async def tax_code_hard_delete(request: Request, tax_code_id: str) -> RedirectResponse:
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
