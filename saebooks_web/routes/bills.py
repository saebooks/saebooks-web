...
async def bill_new_form(
    ...
    async with api_client(request) as client:
        c_resp = await client.get("/api/v1/contacts", params={"type": "SUPPLIER", "limit": 500, "offset": 0})
        if c_resp.is_success:
...
# ---------------------------------------------------------------------------
# Bulk action — POST /bills/bulk
# ---------------------------------------------------------------------------

_BULK_ACTIONS = {
    "post":    ("POST",   "/api/v1/bills/{id}/post"),
    "void":    ("POST",   "/api/v1/bills/{id}/void"),
    "archive": ("DELETE", "/api/v1/bills/{id}"),
}


@router.post("/bills/bulk", response_class=HTMLResponse, response_model=None)
async def bills_bulk_action(request: Request) -> RedirectResponse:
    """Run an action against many bills at once.

    Form fields:
      action  — one of: post / void / archive
      ids[]   — one entry per bill UUID

    Aggregates per-row outcomes into a flash message and redirects back
    to /bills. Best-effort: a 409/422 on one row does not halt the batch.
    """
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
    ...
    async with api_client(request) as client:
        for bill_id in ids:
            try:
                resp = await client.request(method, path_tpl.format(id=bill_id))
                if 200 <= resp.status_code < 300:
                    ...
                    try:
                        ...
                        msg = ""
                    failed.append(f"{bill_id[:8]} ({resp.status_code}{': ' + msg if msg else ''})")
            except Exception as exc:
                failed.append(f"{bill_id[:8]} (transport error: {exc!s})")
    ...
    if failed:
        ...
        request.session["flash"] = f"{label}: {ok} bill{'s' if ok != 1 else ''} processed."
    return RedirectResponse(url="/bills", status_code=303)
...
@router.post("/bills/{bill_id}/hard-delete", response_class=HTMLResponse, response_model=None)
async def bill_hard_delete(request: Request, bill_id: str) -> RedirectResponse:
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
