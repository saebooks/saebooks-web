...
@router.post("/time-entries/bulk", response_class=HTMLResponse, response_model=None)
async def time_entries_bulk_action(request: Request) -> RedirectResponse:
    """Run an action against many time entrys at once.

    Form fields:
      action  — one of: submit, approve, archive
      ids[]   — one entry per UUID

    Aggregates per-row outcomes into a flash message and redirects back
    to /time-entries. Best-effort: a failed row does not halt the batch.
    """
    if not _require_auth(request):
        return RedirectResponse(url="/login", status_code=303)
