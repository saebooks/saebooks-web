...
Defence in depth
----------------
Even with the deployment-level trust model above, this middleware is paired
with a server-side allowlist on the API side (``SAEBOOKS_OAUTH_ALLOWED_EMAILS``
in saebooks). If the allowlist is empty AND ``SAEBOOKS_ENV`` is not
``dev``/``test`` AND ``SAEBOOKS_TRUSTED_HEADER_DEV_OPEN`` is not set, the
oauth-handoff endpoint default-denies with 403 — see
``saebooks/api/v1/login.py``. This middleware handles that 403 by leaving
the request unauthenticated (``not resp.is_success`` → call ``next``
without setting ``api_token``), so the user simply gets the public/login
redirect they would have seen if trusted headers were absent. Do not
change that behaviour without auditing the allowlist contract on the API
side.
"""
